...
*/
define('YOUR_SPECIAL_SECRET_KEY', '5a0da956278367.37519973');
define('YOUR_LICENSE_SERVER_URL','https://technocrackers.com/');
define('YOUR_ITEM_REFERENCE', 'YOUR_ITEM_REFERENCE');
if ( ! defined( 'ABSPATH' ) )
{
	exit;
}
function christmas_greetings_default() 
{
	...
	$user_wp_info = array('theme_name' => $my_theme->get( 'Name' ), 'theme_verson' => $my_theme->get( 'Version' ), 'wp_verson' => get_bloginfo('version'),'blog_name'=>get_option('blogname'),'siteurl'=>get_option('siteurl'));
	$user_wp_info = json_encode($user_wp_info);
	wp_remote_get(YOUR_LICENSE_SERVER_URL.'app/user_data.php?p=christmas_greetings&m='.$user_wp_info.'&d='.get_site_url());
	...
	if($get_set=='') 
	{
		...
		update_option('genral_settings', $christmas_greetings_settings,'','yes' );
		$christmas_greetings_advance_settings = array('fontfamily' => 'Great_Vibes', 'color' => 'color1', 'msg' => 'happy' , 'code' => 'FREE' , 'res_product_select' => '' , 'layout' => 'layout1',);
		$christmas_greetings_advance_settings = json_encode($christmas_greetings_advance_settings);
		update_option('advance_settings', $christmas_greetings_advance_settings,'','yes' );
	}
}
...
{
	$links = array_merge( array('<a href="' . esc_url( admin_url( '/options-general.php?page=christmas_greetings' ) ) . '">' . __( 'Settings', 'textdomain' ) . '</a>'), $links );
	return $links;
}
...
{
	$path =  plugin_dir_url( __FILE__ );
    wp_register_script( 'cpa_custom_js', '/wp-includes/js/jquery/jquery.color.min.js', array( 'jquery', 'wp-color-picker' ), '', true  );
    wp_enqueue_script('cpa_custom_js');
}
...
function christmas_greetings_options() 
{
	...
	$lic5 = new christmas_greetings_lic_class( true, YOUR_LICENSE_SERVER_URL , YOUR_SPECIAL_SECRET_KEY );
	...
	if (isset($_REQUEST['activate_license'])) 
    {
        ...
        $lic    = new christmas_greetings_lic_class($license_key , YOUR_LICENSE_SERVER_URL , YOUR_SPECIAL_SECRET_KEY );
        if($lic->christmas_greetings_active())
        {
        	echo '<div id="message" class="updated fade"><p><strong>'; _e( 'You license Activated successfuly...!!!' ); echo'</strong></p></div>';
        }
        else
        {
        	echo '<div id="message" class="updated fade" style="border-left-color:#a00;"><p><strong>'.$lic->err.'</strong></p></div>';
        }
    }
	if (isset($_REQUEST['deactivate_license'])) 
    {
        $lic = new christmas_greetings_lic_class(true , YOUR_LICENSE_SERVER_URL , YOUR_SPECIAL_SECRET_KEY );
        if($lic->christmas_greetings_deactive())
        {
	...
	echo '
		<form method="post" id="gift_form"action="">
														...
														<div>
															...
															<label for="three">
																<div id="three_img" class="role unselect ';if ( $radio_snow=="three" ){echo 'selected';} echo'" for="three">
																	<input style="display:none" checked class="radio_snow_font" type="radio" name="snow_font" id="snow_font1" value="❄" '; if ( isset ( $radio_font_snow ) ) checked( $radio_font_snow, "❄" ); echo'><label id="snow_lable1" class="snow_lable '; if($radio_font_snow=="❄"){ echo'selected_snow'; } echo ' " for="snow_font1"style=" margin-left:3px; margin-top:3px;" >❆</label>
										...
										<div class="greetings-type">
											...
											<div class="tabcontent03">
												<div class="tc_bg_aa">
													...
													<div id="santa_show-me" class="single-tab-content-wrap image-wrap hide-option ';if($tc_song_radio=='enable_song'){ echo "show-option";	} echo'">
														<div class="tc_padding">
														<!--<div> 
																<h5>song 1</h5>
																<input type="radio" name="song" checked value="song1" id="song_1"'; if ( isset ( $radio_song ) ) checked( $radio_song, "song1" ); echo'>
																<label class="unselect" class="role2" for="song_1">song 1</label>
															</div>
															<div>
																<h5>song 2</h5>
																<input type="radio" name="song" value="song2" id="song_2"'; if ( isset ( $radio_song ) ) checked( $radio_song, "song2" ); echo'>
																<label class="unselect" class="role2" for="song_2">song 2</label>
															</div>-->								
														</div>
													</div>
												</div>
												...
												<div class="greetings-type">
													...
													<div class="tabcontent03">
														<div class="tc_bg_aa">
															...
															<div id="santa_show-me" class="single-tab-content-wrap image-wrap hide-option ';if($new_tc_song_radio=='enable_song'){ echo "show-option";	} echo'">
																<div class="tc_padding">
																<!--<div> 
																		<h5>song 1</h5>
																		<input type="radio" name="new_song" checked value="song1" id="new_one2"'; if ( isset ( $new_radio_song ) ) checked( $new_radio_song, "song1" ); echo'>
																		<label class="role2" for="new_one2">song 1</label>
																	</div>
																	<div>
																		<h5>song 2</h5>
																		<input type="radio" name="new_song" value="song2" id="new_two2"'; if ( isset ( $new_radio_song ) ) checked( $new_radio_song, "song2" ); echo'>
																		<label class="role2" for="new_two2">song 2</label>
																	</div>-->	
																</div>
															</div>
														</div>
																	...
																	<div class="type-product">
																		<div class="row">     
																			<div class="new_dropdown-mul-1">
																				<select name="new_res_product_select[]" style="" id="res_product_select" multiple placeholder="Select"></select>
																			</div>
																		</div>
																	</div>
																</div>
			...
			<p class="submit">
				<input class="button-primary" name="save" type="submit" value="'; _e( "Save Changes" ); echo'" />
			</p>';wp_nonce_field( 'Save_Changes' );	echo '
		</form>
		<p style="text-align:center;">If you like Christmas-Greeting please leave us a <a href="https://wordpress.org/support/plugin/christmas-greetings/reviews/" target="_blank">★★★★★</a> rating. A huge thanks in advance! Version 1.1</p>
	...
	<script type="text/javascript">
		jQuery(window).ready(function() 
		{
			...
			jQuery(".dropdown-mul-1").dropdown(
			{
				data: term,
				limitCount: 40,
				multipleMode: "label",
			...
			jQuery(".new_dropdown-mul-1").dropdown(
			{
				data: new_term,
				limitCount: 40,
				multipleMode: "label",
...
class christmas_greetings_lic_class
{
    ...
    public function __construct($lic=false , $server , $api_key)
    {
   	...
   	public function christmas_greetings_link( $links, $plugin_file, $plugin_data ) 
	{
		if ( isset( $plugin_data['PluginURI'] ) && false !== strpos( $plugin_data['PluginURI'], 'https://wordpress.org/plugins/christmas-greetings/' ) )
		{		
	...
	public function setup() 
	{
		if ( is_admin() ) 
		{
			add_filter( 'plugin_row_meta', array($this,'christmas_greetings_link'), 10, 4 );
		}
	}
    ...
    public function chack_key_status()
    {
    	$path =  plugin_dir_url( __FILE__ );
        if($this->key_status == 'true1')
        {
            $lic2 = unserialize(base64_decode(get_option($this->wp_option)));
            $url = YOUR_LICENSE_SERVER_URL . '/?secret_key=' . YOUR_SPECIAL_SECRET_KEY . '&slm_action=slm_activate&license_key=' . $lic2['l']. '&registered_domain=' . get_site_url().'&item_reference='.$this->product_id;
            $response3 = wp_remote_get($url, array('timeout' => 20, 'sslverify' => false));
	        if(is_array($response3))
	        {
	            $json = $response3['body'];
	            $json = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', utf8_encode($json));
	            $license_data = json_decode($json);
		        if($license_data->error_code == 40 || $license_data->error_code == 110 )
		        {        	
		   			$mkey = base64_encode(serialize(array('l' => $lic2['l'] , 'd' =>strtotime(date('d-m-Y')) , 's' => $license_data->error_code )));
			       	update_option( $this->wp_option, $mkey );
		        }
	        }
        }
        if($this->is_technocrackers())
        {
			...
			<div class="col-50">
				...
				<p class="submit">
	               	<input type="submit" name="deactivate_license" value="Deactive" class="button-primary-active" />
	           	</p>
			</div>';			
        }
        else
    ...
    public function create_key_data($status)
    {
        $mkey = base64_encode(serialize(array('l' => $this->lic, 'd' =>strtotime(date('d-m-Y')) , 's' => $status )));
       	update_option( $this->wp_option, $mkey );
    }
    public function christmas_greetings_active()
    {
        $url = YOUR_LICENSE_SERVER_URL . '/?secret_key=' . YOUR_SPECIAL_SECRET_KEY . '&slm_action=slm_activate&license_key=' . $this->lic . '&registered_domain=' . get_site_url().'&item_reference='.$this->product_id;
        ...
        if($license_data->result == 'success' || $license_data->error_code == 40 || $license_data->error_code ==110)
        {
			...
			$newyear_greetings_settings = json_encode($newyear_greetings_settings);
			update_option('new_genral_settings', $newyear_greetings_settings,'','yes' );
			$newyear_greetings_advance_settings = array('new_fontfamily' => 'Great_Vibes', 'new_color' => 'color1', 'new_msg' => 'happy', 'new_code' => 'FREE' , 'new_res_product_select' => '' , 'new_layout' => 'layout1',);
			$newyear_greetings_advance_settings = json_encode($newyear_greetings_advance_settings);
			update_option('new_advance_settings', $newyear_greetings_advance_settings,'','yes' );
            $this->create_key_data($license_data->message);
            return true;
        }
        else
        {
    ...
    public function christmas_greetings_deactive()
    {
        ...
        $deact_url = YOUR_LICENSE_SERVER_URL . '/?secret_key=' . YOUR_SPECIAL_SECRET_KEY . '&slm_action=slm_deactivate&license_key=' . $lic_data['l'] . '&registered_domain=' . get_site_url();
...
$setup = new christmas_greetings_lic_class( true, YOUR_LICENSE_SERVER_URL , YOUR_SPECIAL_SECRET_KEY );
$setup->setup();
function show_christmas_gift()
{
	$techno_run = new christmas_greetings_lic_class( true, YOUR_LICENSE_SERVER_URL , YOUR_SPECIAL_SECRET_KEY );
